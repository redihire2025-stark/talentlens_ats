...
describe('exportResume', () => {
  it('produces a docx file with a filename derived from the candidate name', async () => {
    const result = await exportResume({ resume: buildTestResume({ contact: { ...buildTestResume().contact, name: 'Jordan Rivera' } }), format: 'docx' })
  ...
  it('falls back to a generic filename when no name was parsed', async () => {
    const resume = buildTestResume({ contact: buildContactInformation({ name: null, email: null, phone: null, location: null, links: [] }) })
